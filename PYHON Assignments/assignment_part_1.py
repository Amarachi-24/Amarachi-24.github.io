...
def read_Image():
    try:
        with open("images1.pgm", 'r') as data_file:# Open the file 'image2.pgm' in read mode and use 'data_file' as a file object
...
# Open the file named 'image2.pgm' in read mode and use 'file' as a file object
    with open('images1.pgm', 'r') as file:
...
def decode_image():
    ...
    for line in pixel_lines: # Iterate over each line of pixel data
            for pixel in line.split():# Iterate over each pixel value in the line
                if int(pixel) == 110:# Check if the pixel value is equal to the message pixel value
...
def write_decoded_image(decoded_message, max_pixel_value):
    try:
        ...
        with open("decodedimage1.pgm", 'w') as decoded_file:
        ...
        print("Decoded image file 'decodedimage1.pgm' created successfully.")
